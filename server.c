...
void accept_request(int newsock) {
	...
	do {
		...
		if (strstr(request, "Connection: close") != NULL)
			connection = "close";
		else
...
int openCon(int sock, struct sockaddr_in server, struct sockaddr *serverptr,
		int serverlen, int port) {
...
while (1) {
	...
	accept_request(newsock);

//		if ((err = pthread_create(&new_thread, NULL, accept_request, newsock))
//				!= 0) {
