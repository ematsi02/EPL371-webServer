...
int openCon(int sock,struct sockaddr_in self, struct sockaddr_in server, struct sockaddr *serverptr,int serverlen,int port){
...
int sk= openCon(sock,self,server,serverptr,serverlen,port);
  ...
  while(1) {
   ...
   if(strcmp(method,"GET")==0){
	  ...
	  sprintf(mybuf,"%s501 Not Implemented%s%s%d%skeep-alive%stext/plain%s",answerCode,serverName,contentLen,strlen(content501),connection,contentType,content501);
	   write(newsock,mybuf,strlen(mybuf));
